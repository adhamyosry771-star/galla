      ...
      >
        <header className="p-6 border-b border-white/5 flex justify-between items-center bg-amber-600/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-white font-black text-sm">بلاغات الغرف</h3>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {loading ? (
            ...
            <div className="text-center py-20 text-white/20 text-xs">لا يوجد بلاغات حالياً</div>
          ) : reports.map(report => (
            <div key={report.id} className="bg-white/5 border border-white/5 rounded-3xl p-4 space-y-4 animate-in slide-in-from-bottom-2 duration-300">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-amber-500/10 flex items-center justify-center text-amber-400 border border-white/5">
                    <i className="fas fa-bullhorn text-sm"></i>
                  </div>
                  <div>
                    <p className="text-[11px] font-black text-white">{report.roomName}</p>
                    <p className="text-[9px] font-bold text-white/40">Room ID: {report.roomId}</p>
                  </div>
                </div>
                <button 
                  onClick={() => handleRemoveReport(report.id)}
                  className="w-8 h-8 rounded-lg bg-red-500/10 text-red-500 flex items-center justify-center active:scale-90 transition-all border border-red-500/10"
                >
                  <i className="fas fa-trash-alt text-[10px]"></i>
                </button>
              </div>

              <div className="p-3 bg-black/20 rounded-2xl space-y-3">
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-black text-amber-500/60 transition-all">السبب:</span>
                ...
                <div className="pt-2 border-t border-white/5 grid grid-cols-2 gap-2">
                  <div className="flex flex-col">
                    <span className="text-[8px] font-black text-white/20 uppercase">صاحب الغرفة</span>
                    <span className="text-[9px] font-bold text-white/60">ID: {report.roomOwnerCustomId || 'مجهول'}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[8px] font-black text-white/20 uppercase">المُبلغ</span>
                    <span className="text-[9px] font-bold text-white/60">ID: {report.reporterCustomId || 'مجهول'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between mt-1 px-1">
                <div className="flex items-center gap-1.5 text-white/20">
                  ...
                  <span className="text-[8px] font-bold">{report.createdAt?.toDate ? report.createdAt.toDate().toLocaleString('ar-EG') : 'جاري التحميل...'}</span>
                </div>
